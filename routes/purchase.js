...
const purchaseRoutes = {
  async getTicketDetails(req, res) {
    try {
      ...
      const purchase = await Purchase.findOne({ _id: id, _user: _id }).populate("_event _user")
      if (purchase === null) {
        return res.status(400).json({ error: true, reason: "transaction not found with this id" })
      }
  ...
  async buyTicket(req, res) {
    try {
      ...
      if (existingEvent === null) {
        return res.status(400).json({ error: true, reason: "Event not found with this Id" })
      }

      if (quantity === undefined) {
        return res.status(400).json({ error: true, reason: "Field 'quantity' is required" })
      }
